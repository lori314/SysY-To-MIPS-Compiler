...
> SysY 是 C 语言的一个教学子集。本仓库来自编译课程实践，保留了后续优化阶段中的一些实验性实现。
...
仓库中也保留了完整 `Mem2Reg`、Phi 相关处理等实验实现，但当前默认 pipeline 中没有启用。代码中的注释记录了关闭它们的原因：跨函数引用场景下仍存在正确性问题。因此 README 只把当前实际执行的 Pass 列为默认优化流程。
...
这个仓库保留了课程开发后期对优化器和后端的进一步尝试，因此同时包含“稳定主流程”和“实验代码”。
...
这些实现没有从仓库中删除，便于继续比较不同优化和寄存器分配方案。
